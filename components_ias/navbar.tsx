...
import { siteConfig } from "@/config_ias/site";
import { ThemeSwitch } from "@/components_ias/theme-switch";
...
export const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      >
        <NavbarBrand as="li" className="justify-start w-auto">
          <NextLink className="flex items-center gap-3" href="/ias">
            <div className="flex items-center gap-2">
              {/* IAS logo light (bigger, no radius) */}
              <Image
                isBlurred={false}
                src="/images/logos/ias.png"
                alt="IEEE IAS Logo"
                className="w-20 h-14 rounded-none object-contain dark:hidden"
              />
              {/* IAS logo dark (bigger, no radius) */}
              <Image
                isBlurred={false}
                src="/images/logos/ias-logo-white.png"
                alt="IEEE IAS Logo (Dark)"
                className="w-20 h-14 rounded-none object-contain hidden dark:block"
              />
            </div>
          </NextLink>
        </NavbarBrand>
            ...
            <NavbarItem key={item.href}>
              <NextLink
                className={clsx(
                  ...
                  "data-[active=true]:text-green-700 data-[active=true]:font-semibold justify-center"
                )}
                href={item.href}
              >
                {item.label}
        ...
        <NavbarItem className="hidden md:flex">
          <Button
            ...
            target="_blank"
            variant="light"
            className="relative font-medium text-white overflow-hidden transition-all duration-500 hover:scale-105 hover:shadow-lg bg-gradient-to-r from-green-600 via-green-500 to-emerald-600 border-none px-6 py-2.5 rounded-xl"
          >
            <span className="relative z-10">Join Us</span>
            <div className="absolute inset-0 bg-gradient-to-r from-green-700 via-green-600 to-green-300 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-xl" />
          </Button>
        </NavbarItem>
        ...
        <div className="flex flex-col justify-center items-center h-full w-full p-8">
          ...
          <div className="flex flex-col items-center gap-2 mb-12">
            {siteConfig.navItems.map((item, index) => (
              ...
              >
                <NextLink
                  className="text-3xl font-medium text-green-600 dark:text-gray-200 py-4 px-8 block transition-all duration-300 hover:text-green-600 dark:hover:text-green-400"
                  href={item.href}
                  onClick={() => setIsMenuOpen(false)}
                >
                  {item.label}
          ...
          >
            <Button
              ...
              href="https://ieee.org/membership/join"
              target="_blank"
              className="relative group font-semibold text-green-600 bg-white hover:bg-green-50 border-none px-10 py-4 text-lg rounded-2xl shadow-2xl transition-all duration-500 hover:scale-110 hover:shadow-green-500/25 overflow-hidden"
              onClick={() => setIsMenuOpen(false)}
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-green-600 to-emerald-600 opacity-0 group-hover:opacity-10 transition-opacity duration-300" />
            </Button>
          </NavbarMenuItem>
          ...
          >
            <div className=" text-green-700 text-sm text-center">
              <p className="font-light">IEEE IAS ISTIC Chapter</p>
